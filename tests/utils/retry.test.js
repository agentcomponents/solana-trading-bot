...
import { retry, retryIf, retryWithTimeout, DEFAULT_RETRY_OPTIONS } from '../../src/utils/retry';
...
describe('Retry Utility', () => {
    describe('retry', () => {
        ...
        it('should handle all attempts failing', async () => {
            ...
            await expect(retry(fn, { maxAttempts: 3, initialDelayMs: 10 })).rejects.toThrow('persistent error');
            expect(fn).toHaveBeenCalledTimes(3);
        });
    });
    describe('retryIf', () => {
        it('should only retry when predicate returns true', async () => {
            ...
            await expect(retryIf(fn, (err) => err.message === 'retryable', { maxAttempts: 5, initialDelayMs: 10 })).rejects.toThrow('fatal');
            expect(fn).toHaveBeenCalledTimes(2);
        });
        ...
        it('should retry until success on retryable errors', async () => {
            ...
            const fn = vi
                ...
                .mockRejectedValueOnce(retryableError)
                .mockResolvedValue('success');
            const result = await retryIf(fn, (err) => err.message === 'retryable', { maxAttempts: 5, initialDelayMs: 10, jitter: false });
            expect(result).toBe('success');
            expect(fn).toHaveBeenCalledTimes(3);
        });
        it('should retry based on error type', async () => {
            ...
            const fn = vi
                ...
                .mockResolvedValue('success');
            const result = await retryIf(fn, (err) => err instanceof NetworkError, { maxAttempts: 5, initialDelayMs: 10 });
            expect(result).toBe('success');
            expect(fn).toHaveBeenCalledTimes(3);
        });
    });
    ...
    describe('error handling', () => {
        it('should convert non-Error errors to Error', async () => {
            ...
            await expect(retry(fn, { maxAttempts: 2, initialDelayMs: 10 })).rejects.toThrow('string error');
        });
        it('should handle null error', async () => {
            const fn = vi.fn().mockRejectedValue(null);
            await expect(retry(fn, { maxAttempts: 2, initialDelayMs: 10 })).rejects.toThrow();
        });
